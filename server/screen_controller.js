...
exports.getServingTickets = async (req, res) => {
  knex
    ...
    .where({ Status: "S" })
    .then((rows) => {
      let resJson = rows.map((row) =>
        createTicket(row.TID, row.Type, row.Time, row.CID, row.Status, row.SID)
      );
      res.json(resJson);
    })
    .catch((err) =>
      res.json({
        message: `There was ans error while retriving serving tickets ${err}`,
      })
    );
};
